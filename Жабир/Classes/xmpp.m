...
-(void) cleanUpState
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kJabirAccountStatusChanged object:nil];
            ...
            [self.processQueue addOperationWithBlock:^{
                ...
                if([[stanzaToParse objectForKey:@"stanzaType"]  isEqualToString:@"iq"])
                {
                    ...
                    if(iqNode.vCard)
                    {
                        ...
                        [[NSNotificationCenter defaultCenter] postNotificationName:kJabirContactRefresh object:self userInfo:userDic];
                    ...
                    if ([iqNode.type isEqualToString:kiqResultType])
                    {
                        ...
                        if([iqNode.idval isEqualToString:self.jingle.idval])
                        {
                            ...
                            [[NSNotificationCenter defaultCenter]
                             postNotificationName: kJabirCallStartedNotice object: userDic];
                            ...
                            if([[iqNode.jingleSession objectForKey:@"action"] isEqualToString:@"session-initiate"]) {
                                ...
                                if (pcmaPayload && transport1) {
                                    ...
                                    if(iqNode.user && iqNode.resource && self.fulluser) {
                                        ...
                                        [[NSNotificationCenter defaultCenter]
                                         postNotificationName: kJabirCallRequestNotice object: dic];
                ...
                else  if([[stanzaToParse objectForKey:@"stanzaType"]  isEqualToString:@"message"])
                {
                                                              ...
                                                              if(success)
                                                              {
                                                                  ...
                                                                  [self.networkQueue addOperationWithBlock:^{
                                                                      ...
                                                                      if(messageNode.from  ) {
                                                                          ...
                                                                          [[NSNotificationCenter defaultCenter] postNotificationName:kJabirNewMessageNotice object:self userInfo:userDic];
                                                                      }
                                                                  }];
                                                              }
                                                              else {
                    ...
                    if(messageNode.receivedID)
                    {
                        ...
                        //Post notice
                        [[NSNotificationCenter defaultCenter] postNotificationName:kJabirMessageReceivedNotice object:self userInfo:@{kMessageId:messageNode.receivedID}];
                        ...
                        if(presenceNode.type ==nil)
                        {
                                                              ...
                                                              [NSCharacterSet whitespaceAndNewlineCharacterSet]] length]>0))
                            {
                                ...
                                [[DataLayer sharedInstance] addContact:presenceNode.user forAccount:self->_accountNo fullname:@"" nickname:@"" withCompletion:^(BOOL success) {
                                    ...
                                    [[NSNotificationCenter defaultCenter] postNotificationName:kJabirContactOnlineNotice object:self userInfo:userDic];
                        ...
                        else if([presenceNode.type isEqualToString:kpresenceUnavailable])
                        {
                            if ([[DataLayer sharedInstance] setOfflineBuddy:presenceNode forAccount:self->_accountNo] ) {
                                ...
                                [self.networkQueue addOperationWithBlock: ^{
                                    ...
                                    [[NSNotificationCenter defaultCenter] postNotificationName:kJabirContactOfflineNotice object:self userInfo:userDic];
                                }];
                            }
...
-(NSString*)getVersionString
{
    ...
    NSString* unhashed=[NSString stringWithFormat:@"client/phone//Jabir %@<%@", [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"], [XMPPIQ featuresString]];
    ...
    for(MLXMLNode* node in _outputQueue)
    {
        ...
        if(success) {
            if([node isKindOfClass:[XMPPMessage class]])
            {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName: kJabirSentMessageNotice object:self userInfo:dic];
