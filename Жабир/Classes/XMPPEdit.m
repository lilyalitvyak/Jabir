...
- (void)viewDidLoad
{
    ...
    if(_originIndex.section==0)
    {
        ...
        NSString*pass= [SAMKeychain passwordForService:@"Jabir" account:[NSString stringWithFormat:@"%@",self.accountno]];
...
-(IBAction) save:(id) sender
{
    ...
    [dic setObject:@"Jabir-iOS" forKey:kResource];
                ...
                if(result) {
                    [[DataLayer sharedInstance] executeScalar:@"select max(account_id) from account" withCompletion:^(NSObject * accountid) {
                        if(accountid) {
                            ...
                            [SAMKeychain setPassword:self.password forService:@"Jabir" account:self.accountno];
                            if(self.enabled)
                            {
...
- (IBAction) delClicked: (id) sender
{
    ...
    UIAlertAction *yesAction =[UIAlertAction actionWithTitle:@"Yes" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        [SAMKeychain deletePasswordForService:@"Jabir"  account:[NSString stringWithFormat:@"%@",self.accountno]];
        ...
        switch (indexPath.row)
        {
            ...
            case 1: {
                if([self.accountType isEqualToString:kGtalk]){
                    ...
                    buttonCell.buttonText.textColor= jabirGreen;
