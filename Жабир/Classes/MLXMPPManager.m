...
-(id) init
{
    ...
    _netQueue = dispatch_queue_create(kJabirNetQueue, DISPATCH_QUEUE_SERIAL);
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleSentMessage:) name:kJabirSentMessageNotice object:nil];
...
-(void) connectAccountWithDictionary:(NSDictionary*)account
{
    ...
    xmppAccount.password = [SAMKeychain passwordForService:@"Jabir" account:[NSString stringWithFormat:@"%@",[account objectForKey:kAccountID]]];
                       ...
                       dispatch_source_set_event_handler(sendTimer, ^{
                           ...
                           for(NSDictionary *dic in  self.timerList) {
                               if([dic objectForKey:kSendTimer] == sendTimer) {
                                   ...
                                   [[NSNotificationCenter defaultCenter] postNotificationName:kJabirSendFailedMessageNotice object:self userInfo:dic];
