...
-(void) viewWillAppear {
    ...
    if(!self.accountToEdit) {
       ...
       // self.resource.stringValue =[self.accountToEdit objectForKey:kResource];
        ...
        NSString *pass= [SAMKeychain passwordForService:@"Jabir" account:[NSString stringWithFormat:@"%@",[self.accountToEdit objectForKey:kAccountID]]];
...
//    }
        ...
        [[DataLayer sharedInstance] addAccountWithDictionary:dic andCompletion:^(BOOL result) {
            if(result) {
                [[DataLayer sharedInstance] executeScalar:@"select max(account_id) from account" withCompletion:^(NSObject * accountid) {
                    if(accountid) {
                        [SAMKeychain setPassword:passwordText forService:@"Jabir" account:[NSString stringWithFormat:@"%@", accountid]];
    ...
    else
    {
        [[DataLayer sharedInstance] updateAccounWithDictionary:dic andCompletion:^(BOOL result) {
            [self refreshPresenter];
            [SAMKeychain setPassword:passwordText forService:@"Jabir" account:[NSString stringWithFormat:@"%@", [self.accountToEdit objectForKey:kAccountID]]];
